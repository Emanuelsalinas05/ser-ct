...
@section('title', '13. ARCHIVOS')
@section('content_header_title', 'Home')
@section('content_header_subtitle', ' 13. ARCHIVOS')
                    ...
                    @foreach($documentos as $documento)
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            @foreach($avanceanexos as $avance)
                                ...
                                @elseif($documento->oavance_documentos=='orelacion_archivos_historico_a')
                                    <a  href="{{ url($documento->ourl_documentos) }}"
                                        style="text-decoration: none; font-size:12px;" 
                                        class="btn bg-teal btn-outline-dark btn-sm ">
                                        @if($avance->orelacion_archivos_historico_a==1)
                                            Completado   
                                            <i class="fas fa-check"></i>    
                                        @else
                                            Registrar  
                                ...
                                @elseif($documento->oavance_documentos=='orelacion_documentos_noconvencionles_a')
                                    <a  href="{{ url($documento->ourl_documentos) }}"
                                        style="text-decoration: none; font-size:12px;" 
                                        class="btn bg-teal btn-outline-dark btn-sm ">
                                        @if($avance->orelacion_documentos_noconvencionles_a==1)
                                            Completado   
                                            <i class="fas fa-check"></i>    
                                        @else
                                            Registrar  
