...
@section('title', '5. RECURSOS HUMANOS')
@section('content_header_title', 'Home')
@section('content_header_subtitle', ' 5. RECURSOS HUMANOS')
                    ...
                    <ul class="list-group list-group-flush"
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            @foreach($avanceanexos as $avance)
                                @if($documento->oavance_documentos=='oplantilla_personal_a')
                                    <a  href="{{ url($documento->ourl_documentos) }}"
                                        style="text-decoration: none; font-size:12px;" 
                                        class="btn bg-teal btn-outline-dark btn-sm ">
                                        @if($avance->oplantilla_personal_a==1)
                                            Completado 
                                ...
                                @elseif($documento->oavance_documentos=='oplantilla_comisionados_a')
                                    <a  href="{{ url($documento->ourl_documentos) }}"
                                        style="text-decoration: none; font-size:12px;" 
                                        class="btn bg-teal btn-outline-dark btn-sm ">
                                        @if($avance->oplantilla_comisionados_a==1)
                                            Completado   
                                            <i class="fas fa-check"></i>    
                                        @else
                                            Registrar  
                        ...
                        @if( $datosacta->avances->oplantilla_personal_a==1 && $datosacta->avances->oplantilla_comisionados_a==1 )
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            &nbsp;

                            @if($datosacta->avances->orecursos_humanos_d==1)
                                <a  href="{{ url('entrega-recepcion') }}" 
