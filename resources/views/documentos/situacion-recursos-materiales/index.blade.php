...
@section('content_header_subtitle', ' 8. SITUACIÓN DE LOS RECURSOS MATERIALES')
        ...
        <x-adminlte-callout theme="info">
            <div class="row">
                <div class="col-sm">
                    ...
                    @foreach($documentos as $documento)
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            @foreach($avanceanexos as $avance)
                                @if($documento->oavance_documentos=='oinventario_bienes_a')
                                    <a  href="{{ url($documento->ourl_documentos) }}"
                                        style="text-decoration: none; font-size:12px;" 
                                        class="btn bg-teal btn-outline-dark btn-sm col-sm-2">
                                        @if($avance->oinventario_bienes_a==1)
                                            Completado 
                                ...
                                @elseif($documento->oavance_documentos=='oinventario_almacen_a')
                                    <a  href="{{ url($documento->ourl_documentos) }}"
                                        style="text-decoration: none; font-size:12px;" 
                                        class="btn bg-teal btn-outline-dark btn-sm col-sm-2">
                                        @if($avance->oinventario_almacen_a==1)
                                            Completado   
                                            <i class="fas fa-check"></i>    
                                        @else
                                            Registrar  
                                ...
                                @elseif($documento->oavance_documentos=='orelacion_bienes_custodia_a')
                                    @if($documento->id==6&&$avance->orelacion_bienes_custodia_a==1)
                                    <a  href="{{ url($documento->ourl_documentos) }}"
                                        style="text-decoration: none; font-size:12px;" 
                                        class="btn bg-teal btn-outline-dark btn-sm col-sm-2">
                                        @if($avance->orelacion_bienes_custodia_a==1)
                                            Completado   
                                            <i class="fas fa-check"></i>    
                                        @else
                                            Registrar  
                    ...
                    @if($datosacta->avances->oinventario_bienes_a==1 && $datosacta->avances->oinventario_almacen_a==1 && $datosacta->avances->orelacion_bienes_custodia_a==1)
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            &nbsp;

                            @if($datosacta->avances->osituacion_recursos_materiales_d==1)
                                <a  href="{{ url('entrega-recepcion') }}" 
