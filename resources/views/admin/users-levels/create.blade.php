    ...
    <div class="card-header bg-light shadow-sm d-flex mb-2">
        <div class="d-flex justify-content-between">
            <b><i class="nav-icon fa fa-users"></i>&nbsp;
                REGISTRO DE USUARIOS PARA APROBACIÓN DE ENTREGA-RECEPCIÓN
            </b> 
        </div>
    </div>
                ...
                <tr>
                    ...
                    <td colspan="3">
                        <select id="oct" 
                            ...
                            @foreach($organitationx as $orgctx)
                            <option value="{{$orgctx->kcvect}}">
                                {{$orgctx->cct_departamento.' - '.$orgctx->onombre_ct.'  --- (VALLE DE '.$orgctx->ovalle.') '}}
                            </option>
                            @endforeach

                            @foreach($organitationa as $orgcta)
                            <option value="{{$orgcta->kcvect}}">
                                {{$orgcta->cct_sector.' - '.$orgcta->onombre_ct.'  --- (VALLE DE '.$orgcta->ovalle.') '}}
                            </option>
                            @endforeach

                            @foreach($organitationb as $orgctb)
                            <option value="{{$orgctb->kcvect}}">
                                {{$orgctb->cct_supervision.' - '.$orgctb->onombre_ct.'  --- (VALLE DE '.$orgctb->ovalle.') '}}
                            </option>
                            @endforeach
