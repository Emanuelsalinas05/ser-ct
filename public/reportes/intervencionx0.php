...
// Datos de conexión y reporte
$filename   = "SOLICITUD_INTERVENCION_E_R.pdf";
$reportFile = "intervencion0.jasper";
...
try {
    ...
    if (empty($pdf) || strlen($pdf) < 1000) {
        file_put_contents("warning_intervencionx0_log.txt", "?? PDF vacío para id3p=$id3p, fecha=$f3c4\n", FILE_APPEND);
    ...
    file_put_contents("error_intervencionx0_java.txt", $ex);
    http_response_code(500);
    exit("Java error");
} catch (Exception $ex) {
    file_put_contents("error_intervencionx0_php.txt", $ex);
    http_response_code(500);
    exit("PHP error");
}
?>
