...
try {
    ...
    if (empty($pdf) || strlen($pdf) < 1000) {
        file_put_contents("warning_noadeudo_log.txt", "?? PDF vacío para iduser=$iduser\n", FILE_APPEND);
        exit("?? PDF no generado o vacío.");
    }

    $filename = "CERTIFICADO_DE_NO_ADEUDO_" . $iduser . ".pdf";
    ...
    file_put_contents("error_certificado_noadeudo_java.txt", $ex);
    http_response_code(500);
    exit("Java error");
} catch (Exception $ex) {
    file_put_contents("error_certificado_noadeudo_php.txt", $ex);
    http_response_code(500);
    exit("PHP error");
}
?>
