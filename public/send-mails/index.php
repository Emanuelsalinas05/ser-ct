...
if ($datosacta->oenviocorreooic == 0) {
    ...
    // NUEVO remitente institucional
    $mail->From = "entregasrecepcion.elemental@seiem.edu.mx";
    ...
    */
    ...
    if (Auth::user()->onivel == 'ELEMENTAL') {
        ...
        if ($ocorreosub != NULL)         $mail->addAddress($ocorreosub);
        if ($ocorreodep != NULL)         $mail->addAddress($ocorreodep);
        if ($ocorreoestructura != NULL)  $mail->addAddress($ocorreoestructura);
        if ($ocorreoestructura2 != NULL) $mail->addAddress($ocorreoestructura2);
