    ...
    public function index()
    {
        ...
        if (Auth::user()->orol == 3) {
            ...
            if (!$intervencionExistente) {
                $intervencionPermitida = false; // No hay intervención generada
            }
        }
        ...
        if (Auth::user()->orol == 3) {
            ...
            if ($elacta) {
                $datosacta = DatosActa::select('*',
                    DB::raw("CASE WHEN id_tipoacta=1 THEN CASE WHEN onombre_entrega_a IS NOT NULL AND orfc_entrega_a IS NOT NULL AND ocargo_entrega_a IS NOT NULL AND onombre_recibe_a IS NOT NULL AND orfc_recibe_a IS NOT NULL AND ocargo_recibe_a IS NOT NULL THEN 1 ELSE 0 END WHEN id_tipoacta=2 THEN CASE WHEN onombre_recibe_ac IS NOT NULL AND orfc_recibe_ac IS NOT NULL THEN 1 ELSE 0 END END AS ock"),
                    DB::raw("CASE WHEN owaitacta=2 AND ocargaacta=0 AND ocargacomprimido=0 THEN 0 WHEN owaitacta=1 AND ocargaacta=1 AND ocargacomprimido=0 THEN 1 WHEN owaitacta=1 AND ocargaacta=1 AND ocargacomprimido=1 THEN 2 END AS avancez"),
                    DB::raw("CASE WHEN ocargacomprimido=0 AND onombrecarpeta IS NULL AND ocorreocc IS NULL THEN 0 WHEN ocargacomprimido=1 AND onombrecarpeta IS NOT NULL AND ocorreocc IS NULL THEN 1 WHEN ocargacomprimido=1 AND onombrecarpeta IS NOT NULL AND ocorreocc IS NOT NULL THEN 2 END AS carpetacorreo")
                )->whereIdUser(Auth::user()->id)->whereOconcluida(0)->first();

                $avanceanexos = Avanceanexos::select('*',
                    DB::raw("CASE WHEN omarco_juridico_d=1 AND orecursos_humanos_d=1 AND osituacion_recursos_materiales_d=1 AND osituacion_tics_d AND ocertificados_no_adeudos_d=1 AND oinforme_gestion_d=1 AND ootros_hechos_d=1 THEN 1 ELSE 0 END AS completado")
                ...
                if ($datosacta->id_tipoacta == 2) {
                    ...
                    $avance = Avanceanexos::select('*',
                        DB::raw("CASE WHEN omarco_juridico_d=1 AND orecursos_humanos_d=1 AND osituacion_recursos_materiales_d=1 AND osituacion_tics_d AND oarchivos_d=1 AND ootros_hechos_d=1 THEN 1 ELSE 0 END AS completado")
                    )->whereIdActa($datosacta->id)->first();
                } else {
                    $anexos = Anexos::orderBy('onum_anexo', 'ASC')->get();
                    $avance = Avanceanexos::select('*',
                        DB::raw("CASE WHEN omarco_juridico_d=1 AND orecursos_humanos_d=1 AND osituacion_recursos_materiales_d=1 AND osituacion_tics_d AND oarchivos_d=1 AND ocertificados_no_adeudos_d=1 AND oinforme_gestion_d=1 AND ootros_hechos_d=1 THEN 1 ELSE 0 END AS completado")
                    )->whereIdActa($datosacta->id)->first();
                }

                $ban = 1;
                return view('acta.index', compact('tipoacta', 'anexos', 'documentos', 'datosacta', 'avanceanexos', 'avance', 'ban', 'us', 'ctts', 'intervencionPermitida'));
            } else {
                $ban = 0;
                return view('acta.index', compact('tipoacta', 'documentos', 'ban', 'us', 'ctts', 'intervencionPermitida'));
            }
        } elseif (Auth::user()->orol <= 2) {
            return redirect(url('entregas-recepcion'));
        } elseif (Auth::user()->orol > 3) {
